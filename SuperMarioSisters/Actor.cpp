...
void Peach::BonkMe(bool bonkerIsStarPowerPeach)
{
	...
	{
		m_hp--;
		TempInvincibility(40);
		m_firePower = false;
		m_jumpPower = false;
		if (getWorld()->getScore() >= 50)
		{
...
void Peach::doSomething()
{
	...
	if (m_remaining_jump_distance == 0)
	{
		falling = fall(2, 2); //checks if anything x pixels below blocking, then moves y pixels down
		if (falling)
		{
			//check for stomp
		}
	...
	if (world->getKey(key))
	{
		...
		case KEY_PRESS_SPACE:
			...
			m_ticks_left = 0; m_ticks_right = 0;
	...
	{
		if (m_ticks_left <= 19)
			world->moveIfPossible(getX() - 2, getY(), this);
		else if (m_ticks_left >= 20)
			world->moveIfPossible(getX() - 4, getY(), this);
	}
	else if (m_ticks_right > 0)
	{
		if (m_ticks_right <= 19)
			world->moveIfPossible(getX() + 2, getY(), this);
		else if (m_ticks_right >= 20)
			world->moveIfPossible(getX() + 4, getY(), this);
	}
...
void Enemy::doSomething()
{
	...
	if (m_stompable)
	{
		//if Peach is directly above me, or if Peach
		if ((abs(world->dYfromPeach(this)) <= SPRITE_HEIGHT) && 
			(abs(world->dYfromPeach(this)) >= (SPRITE_HEIGHT)) && (world->dYfromPeach(this) <= 0))
		{
			//if Peach is within 1 * my width
			if (abs(world->dXfromPeach(this)) <= (SPRITE_WIDTH * 3/4))
			{
				if (world->peachIsFalling())
				{
