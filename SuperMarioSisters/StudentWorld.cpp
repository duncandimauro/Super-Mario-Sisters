...
int StudentWorld::init()
{
    ...
    //my only levels are 1, 2, and 3 with file names "level01.txt", "level02.txt", and "level03.txt"
    ...
    for (int x = 0; x < GRID_WIDTH; x++)
    {
        for (int y = 0; y < GRID_HEIGHT; y++)
        {
            ...
            case Level::GridEntry::block:
                m_actorList.push_back(new Block(x, y, Block::none, IID_BLOCK, this));
                break;
            case Level::GridEntry::flower_goodie_block:
                m_actorList.push_back(new Block(x, y, Block::flower, IID_BLOCK_POWERUP, this));
                break;
            case Level::GridEntry::mushroom_goodie_block:
                m_actorList.push_back(new Block(x, y, Block::mushroom, IID_BLOCK_POWERUP, this));
                break;
            case Level::GridEntry::star_goodie_block:
                m_actorList.push_back(new Block(x, y, Block::star, IID_BLOCK_POWERUP, this));
                break;
...
bool StudentWorld::moveIfPossible(int x, int y, Actor* self)
{
    if (!spaceBlocked(x, y, self)) //if the space isn't blocked
    {
        self->moveTo(x, y);
...
bool StudentWorld::peachIsFalling()
{
    if (!m_peach->isJumping())
    {
        if (m_peach->fall(1, 0))
        {
            return true;
