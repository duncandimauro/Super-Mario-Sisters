...
static const int MS_PER_FRAME = 8; //CHANGE THIS FOR TICK RATE
...
enum GameController::GameControllerState : int {
	welcome, contgame, finishedlevel, init, cleanup, makemove, animate, gameover, prompt, quit, not_applicable, restart, startOver
};
...
void GameController::initDrawersAndSounds()
{
	...
	{ IID_BLOCK_POWERUP, 0, "wall_powerup.tga", "BLOCK_POWERUP" },
...
void GameController::setDirection(int d)
{
	playerDirection = d;
}
...
{
	//Getting the current key being pressed (if there is one)
	int pressedKey;
	Game().getLastKey(pressedKey);

	//Getting the key being lifted up
	Game().keyboardEvent(key, x, y);
	int liftedKey;
	Game().getLastKey(liftedKey);

	//cerr << "pressed Key: " << pressedKey << endl;  //FOR TESTING
	//cerr << "lifted Key: " << liftedKey << endl;    //FOR TESTING

	if (pressedKey == liftedKey)
	{
		//if (pressedKey == KEY_PRESS_RIGHT || pressedKey == KEY_PRESS_LEFT)
		//{
			//playerDirection
		//}
		Game().specialKeyboardEvent(INVALID_KEY, x, y); //Peach stops whatever she was doing
	}
	else
	{
		Game().specialKeyboardEvent(pressedKey, x, y); //specialKeyboardEvent called - deals with ints
	}
}
...
static void specialKeyboardUpEventCallback(int key, int x, int y)
{
	...
	if (pressedKey == liftedKey)
	{
		Game().specialKeyboardEvent(INVALID_KEY, x, y); //Peach stops all actions
	}
	else
	{
		Game().specialKeyboardEvent(pressedKey, x, y);
	}
}
/////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////         NEW CODE END         /////////////////////////////////////
...
void GameController::keyboardEvent(unsigned char key, int /* x */, int /* y */)
{
	...
	case 'r':			if (m_lastKeyHit != KEY_PRESS_R)
		                {setGameState(cleanup);
						m_lastKeyHit = KEY_PRESS_R;}    break;
	case 'q': case 'Q': setGameState(quit);				break;
	case '\x1b':        setGameState(quit);             break; //pressing the escape key
	case '\r':          m_lastKeyHit = KEY_PRESS_ENTER; break;
	case '\t':          m_lastKeyHit = KEY_PRESS_TAB;   break;
	case ' ':           m_lastKeyHit = KEY_PRESS_SPACE; break;
	default:			m_lastKeyHit = INVALID_KEY;		break; //Changed from = key to = INVALID_KEY
	}
}
...
void GameController::specialKeyboardEvent(int key, int /* x */, int /* y */)
{
	switch (key)
	{
	case GLUT_KEY_LEFT:	 case KEY_PRESS_LEFT:   m_lastKeyHit = KEY_PRESS_LEFT;	 break;
	case GLUT_KEY_RIGHT: case KEY_PRESS_RIGHT:  m_lastKeyHit = KEY_PRESS_RIGHT;  break;
	case GLUT_KEY_UP:	 case KEY_PRESS_UP:     m_lastKeyHit = KEY_PRESS_UP;	 break;
	case GLUT_KEY_DOWN:	 case KEY_PRESS_DOWN:   m_lastKeyHit = KEY_PRESS_DOWN;	 break;
	                     case KEY_PRESS_SPACE:  m_lastKeyHit = KEY_PRESS_SPACE;  break;
...
void GameController::doSomething()
{
	...
	case welcome:
		...
		m_mainMessage = "Welcome to Super Mario Sisters!";
	...
	case contgame:
		...
		break;
	case finishedlevel:
		m_mainMessage = "Wahoo! You finished the level!";
		m_secondMessage = "Press Enter to continue playing...";
		setGameState(prompt);
		m_nextStateAfterPrompt = cleanup;
		break;
	case makemove:
		...
		m_nextStateAfterAnimate = not_applicable;
		{
			...
			if (status == GWSTATUS_PLAYER_DIED)
			{
				...
				{
					m_gw->goToFirstLevel();
					m_nextStateAfterAnimate = restart;
				}
				else
				{
					m_nextStateAfterAnimate = contgame;
				}
			}
	...
	case cleanup:
		...
		break;
	case restart:
		m_mainMessage = "Game Over";
		m_secondMessage = "Press Enter to restart, press Q to quit";
		setGameState(prompt);
		m_nextStateAfterPrompt = startOver;
		break;
	case startOver:
		m_gw->cleanUp();
		m_gw->setLives(3);
		m_gw->setScore(0);
		setGameState(prompt);
		m_nextStateAfterPrompt = welcome;
		break;
	case gameover:
	{
		ostringstream oss;
